...
function isPalindrome(str) {
  const reversed = str
    ...
    .toLowerCase()
    .replace(/[\W_]/g, '');
  if (str.replace(/[\W_]/g, '').toLowerCase() === reversed) {
    return true;
  }
...
function invertCase(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    const isUpperCase = str[i].toUpperCase();
...
function encodeToRot13(str) {
  ...
  const originalAlphabeth =
    'abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ';
  const encodedAlphabeth =
    'nopqrstuvwxyzabcdefghijklmNOPQRSTUVWXYZABCDEFGHIJKLM';

  for (let i = 0; i < str.length; i += 1) {
    const index = originalAlphabeth.indexOf(str[i]);

    if (str[i].match(/[a-z]/i)) {
      result += encodedAlphabeth[index];
    } else {
      result += str[i];
    }
  }
